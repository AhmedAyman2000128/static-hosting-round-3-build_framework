...
async function uploadFileToAzure(containerName, filePath, blobName) {
  try {
    ...
    console.log(
      `Upload successful for ${blobName}:`,
...
//Docker exec , create dist , delete container
...
docker.container
  .create({
    ...
    HostConfig: {
      Binds: [
        "/home/ahmed/Study/pocs/build_framework/backend/p1:/p1", // Specify the host path and container path
      ],
      Tty: true, // Allocate a pseudo-TTY
      OpenStdin: true,
    },
  ...
  .then((container) => {
    ...
    return container.exec.create({
      AttachStdout: true,
      AttachStderr: true,
      Cmd: ["sh", "-c", `cd /p1 && npm install && npm run build`],
    });
  })
  .then((exec) => {
    return exec.start({ Detach: false, Tty: true }).then((stream) => {
  ...
  .then(() => docker.container.get("p1").delete({ force: "true" }));
